...
def logistic_regression(data, pca_var, final_dat):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df_cp.drop("liked_by_user", axis=1),
        df_cp["liked_by_user"],
        test_size=0.2,
        random_state=42,
    )
    ...
    ret_songs_3 = reduced_df[["track_id", "Logistic Regression"]].sort_values(
...
def main():
    ...
    for i in temp_s:
        dislikes += list(
            set(
                sr.least_vec_cos(i, reduced_1, df).head(20).loc[:, "track_id"].to_list()
            )
        )
    ...
    ids, scores = [], []
    for id, score in zip(
        temp.head(20).loc[:, "track_id"], temp.head(20).loc[:, "Logistic Regression"]
    ):
        if not id in ids:
            ids.append(id)
