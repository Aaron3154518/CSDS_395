...
def naive_bayes(data,pca_var, final_dat):
    ...
    #temp1 = pca_var.transform(temp)
    #ss, pca, temp1 = sr.run_pca(temp, 8)
    reduced_f = final_dat[~final_dat['track_id'].isin(data)]
    df_fin = reduced_f.drop(columns=['track_id'],axis=1,inplace=False)
    ...
    reduced_f['Naive Bayes'] = gnb.predict_proba(df_fin)[:,1]
    # print(sum(reduced_f['Naive Bayes'].tolist()))
    #results = gnb.predict_proba(temp1)[:,1]
    ...
    ret_songs_3 = reduced_f[['track_id','Naive Bayes']].sort_values(by=['Naive Bayes'], ascending=False)
    return(ret_songs_3)
...
#   '6eN1f9KNmiWEhpE2RhQqB5']
    ...
    temp = naive_bayes(filtered_df_1,pca_1, reduced_2)
    ids, scores = [], []
    for id, score in zip(temp.head(20).loc[:, 'track_id'], temp.head(20).loc[:, 'Naive Bayes']):
        if not id in ids:
            ids.append(id)
