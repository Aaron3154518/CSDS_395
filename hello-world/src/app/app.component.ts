...
export class AppComponent implements AfterViewInit, OnDestroy {
  ...
  fileChange(event: any) {
    ...
    fr.onload = (e) => {
      ...
      a.decodeAudioData(fr.result as ArrayBuffer, (data: AudioBuffer) => {
        this.amps = data.getChannelData(0);
        let max = this.amps.reduce(
          (m, a) => (Math.abs(a) > m ? Math.abs(a) : m),
          0
        );
  ...
  draw() {
    ...
    let prog =
      this.audio.nativeElement.currentTime / this.audio.nativeElement.duration;
    this.idx = Math.floor(this.amps.length * prog);

    this.rad = 0.9 * this.rad + 0.1 * this.amps[this.idx];
  ...
  drawSpiral(i1: number, i2: number) {
    ...
    for (
      let i = Math.floor(this.t * 25), n = Math.floor((this.t + dt) * 25);
      i < n;
      i++
    ) {
      this.spirals.push({
        r: Math.random() * this.w,
    ...
    this.spirals.forEach((s: Spiral) => {
      ...
      let trail = [3, 2, 2, 1, 1];
      trail.forEach((t_r: number, i: number) => {
        let r = s.r - i * dr;
  ...
  drawRain(i1: number, i2: number) {
    ...
    this.rain.forEach((r: Rain) => {
      r.x += r.vx * dt;
      r.y += r.vy * dt + 0.5 * this.ay * dt * dt;
      r.vy += this.ay * dt;
    });

    this.rain = this.rain.filter((r: Rain) => r.y < this.w);

    let last_rad = (this.rad - 0.1 * this.amps[i2]) / 0.9;
    ...
    for (let i = 0; i < Math.floor(dr * dr); i++) {
      ...
      this.rain.push({
        x: this.w / 2,
        y: this.w / 2,
        vx: v * Math.cos(theta) * (Math.random() >= 0.5 ? 1 : -1),
        vy: -v * Math.sin(theta),
      });
    }
